function [ particles, obsPoints, nParts ] = ReadSurvivors(path2Files,obsPointNames)
...
%   obsPoints: list of names of the obervation points;
    ...
    %% set up
    if (~exist("obsPointNames","var")), obsPointNames=missing(); end
        ...
        for ii=1:length(files)
            ...
            while (~feof(fileID))
                ...
                elseif (startsWith(tLine,"#"))
                    % new section
                    myFields=split(tLine);
                    if ( all(ismissing(obsPointNames)) | ~isempty(find(contains(obsPointNames,myFields{end}))) )
                        % acquire beam population at the specified obs point
                        %    or at every obs point
    ...
    %% compact storage
    %  only in case all observation points are parsed
    if ( all(ismissing(obsPointNames)) )
