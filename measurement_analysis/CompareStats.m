function CompareStats(BARs,SIGs,INTs,indices,labels)
...
% 
    ...
    for iRow=1:nRows
        for iCol=1:nCols
            ...
            switch iCol
                case 1
                    what=SIGs; myNam="sigma"; myYLab="[mm]";
                case 2
                    what=BARs; myNam="BAR"; myYLab="[mm]";
                case 3
                    what=INTs; myNam="INT"; myYLab="[]";
            end
            for iSet=1:nSets
                if ( iSet>1 ), hold on; end
                plot(iis,what(iis,iRow,iSet),"*-");
            end
            xlabel("ID []"); ylabel(myYLab); grid on; title(sprintf("%s - %s",planes(iRow),myNam));
