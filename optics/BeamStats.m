function [Counts2D,CountsX,CountsY,xb,yb,contours,contourLabels]=BeamStats(particles,what2Analise,declared,units,xbQuery,ybQuery,contours,contourLabels,myTitle)
% 
...
% 
    ...
    if (~exist("units","var") | all(ismissing(units))), units=["m" "" "m" "" "m" ""];  end
    ...
    if (~ismissing(myTitle))
        %      2D histogram
        lHist=true;  BeamStatsShow(Counts2D,CountsX,CountsY,xb,yb,what2Analise,lHist,myTitle,contours,contourLabels,units,declared);
        %      scatter plot
        lHist=false; BeamStatsShow(particles,CountsX,CountsY,xb,yb,what2Analise,lHist,myTitle,contours,contourLabels,units,declared);
    end
