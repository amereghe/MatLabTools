function [bet,alf,emiG,d,dp,sigdppO]=FitOpticsThroughSigmaData(CC,sigs,sigdppI,initConds)
...
%   . initConds (optional): initial guess values, given as optics functions;
% - output:
...
% 
    ...
    if ( size(CC,1)==2 && size(CC,2)==2 )
        Sx=SolveSigSystem(CC,sigs,missing(),initConds);
    ...
    elseif ( size(CC,1)==3 && size(CC,2)==3 )
        if ( exist('sigdppI','var') )
            ...
            % do the actual scan
            for ii=1:nFits
                Sx=SolveSigSystem(CC,sigs,sigdppI(ii),initConds);
