...
%          X(6) = sig_dpp^2
%   . sigs: row vector of measured sigma values (nData,1);
...
%
% see also BuildTransportMatrixForOptics, DecodeOpticsFit, DecodeOrbitFit and SolveCoSystem
% 
    opts.RECT=true;
    if ( size(B,1)==2 )
        A=zeros(length(sigs),3);
        C=BuildTransportMatrixForOptics(B);
        A(:,1)=C(1,1,:);
        A(:,2)=-C(1,2,:);
        A(:,3)=C(1,3,:);
        X=linsolve(A,sigs,opts);
    elseif ( size(B,1)==3 )
        if ( exist('sig_dpp','var') )
            if ( sig_dpp == 0.0 )
                ...
                A=zeros(length(sigs),5);
            end
        else
            A=zeros(length(sigs),6);
        end
        A(:,1)=B(1,1,:).^2;
        A(:,2)=2*B(1,1,:).*B(1,2,:);
        A(:,3)=B(1,2,:).^2;
        if ( exist('sig_dpp','var') )
            ...
            X=linsolve(A,sigs-reshape(B(1,3,:).^2*sig_dpp^2,length(sigs),1),opts);
            X(6)=sig_dpp^2;
        else
            A(:,4)=2*B(1,1,:).*B(1,3,:);
            A(:,5)=2*B(1,2,:).*B(1,3,:);
